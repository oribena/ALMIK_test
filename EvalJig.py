...
class EvalJig:
    ...
    def compute(self, topic, ranking, qrel):
        self.topics.add(topic)
        sr = sorted(ranking, reverse=False)[:self.evaldepth]
        for op in self.ops:
    ...
    def print_scores(self):
        for topic in sorted(map(int, self.topics)):
    ...
    def get_means(self):
        return {str(op): self.means[str(op)] for op in self.ops if self.means[str(op)] is not None}
...
class R_Precision(Measure):
    ...
    def __str__(self): return 'r_precision'

    def compute(self, ranking, qrel, minrel):
        num_rel = sum(1 for rel in qrel.values() if rel >= minrel)
        if num_rel == 0:
            return 0
        s = sum(1 for (rank, score, doc) in rel_ranks(ranking, qrel, minrel) if rank <= num_rel)
...
class RelRet(Measure):
    ...
    def compute(self, ranking, qrel, minrel):
        return sum(1 for (s, d) in ranking if d in qrel and qrel[d] >= minrel)
...
def rel_ranks(ranking, qrels, minrel=1):
    ...
    for (score, doc) in ranking:
        rank += 1
        if doc in qrels and qrels[doc] >= minrel:
...
class MRE(Measure):
    ...
    def compute(self, ranking, qrel, minrel=1):
        try:
            ranks, scores, docs = zip(*rel_ranks(ranking, qrel, minrel))
